...
class TransactionsPage {
  ...
  removeAccount() {
    ...
    const confirmAccountWindow = confirm('Вы действительно хотите удалить счёт?');

    if (!confirmAccountWindow) {
      return;
    }
  ...
  removeTransaction(id) {
    const confirmTransactionWindow = confirm('Вы действительно хотите удалить эту транзакцию?');

    if (!confirmTransactionWindow) {
      return;
    }
    ...
    <div class="transaction ${transactionType} row">
      ...
      <div class="col-md-3">
        <div class="transaction__summ">
          ${Math.abs(item.sum)} <span class="currency">₽</span>
        </div>
      </div>
